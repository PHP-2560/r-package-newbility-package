...
```
```{r}
# This is a classic 24 solver function that allows a user to input 4 cards and gives all of the legal methods of calculating 24. The user can input any value as the objective number
game24=function(A,b=24){
  len=length(A)
  B=permn(A)
  stopifnot(len==4)
  ...
  if(length(result1) == 0) return(FALSE) 
...
##print(paste(B[[s]][1]," ",trans(i)," ",B[[s]][2], " " ,trans(j)," ",B[[s]][3]," ",trans(k)," ",B[[s]][4], " ", "=24"))

game24(c(9,8,8,4))
```

```{r}
# Draw random 5th card and this card becomes the objective card. This game is called Krypto
gameKrypto <- function(A){
  len=length(A)
  B=permn(A)
  b = sample(1:13, 1)
  flag = 0
  ...
  while(flag == 0){
  input <- readline(prompt = "Enter your objective card when you want solutions: ")
...
#  }
  ...
  if(length(result1) == 0) {
    ...
    print("that was not the objective card")
    flag = 0
  }
  }
}
...
```{r}
# This function gives 4 random cards and an objective card, then prompts the user before giving the solutions to Krypto
