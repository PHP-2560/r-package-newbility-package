...
table = function(n) {
  ...
  sample = sample.int(13, 4*n, replace = TRUE)
  sample = matrix(sample, nrow = n, ncol = 4, byrow = T)
  counts = vector(mode = "integer", length = 0)
  
  for (i in 1:n) {
    count = game24(sample[i,], b = 24)
...
colnames(count) = c('Card1', 'Card2', 'Card3', 'Card4', 'Count')

#Calculate the probability of combinations cannot do 24
length(count[count == 0]) / n

ggplot(count, aes(x= Count)) +
  geom_histogram() +
  labs(title = "The count for numbers of calculation methods")
...
```{r}
#game 24 fail, then calculate other number
